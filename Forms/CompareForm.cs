...
namespace Elmanager.Forms
{
    ...
    {
        private readonly Replay[] _comparingRps;
        private readonly int[][] _touchCheckPoints;

        internal CompareForm(Replay[] rps)
        {
            InitializeComponent();
            ...
            foreach (Replay t in rps)
                CRBox.Items.Add(Path.GetFileName(t.Path));
            Array.Resize(ref _touchCheckPoints, rps.Length);
        ...
        {
            int[][] checkPoints;
            double[][] eventTimes = new double[_comparingRps.Length][];
            int[][] appleCheckPoints = new int[_comparingRps.Length][];
            for (int i = 0; i < _comparingRps.Length; i++)
            {
            ...
            for (int i = 0; i < maxCheckPoints; i++)
            {
                ...
                for (var x = 0; x < _comparingRps.Length; x++)
                {
                    double time;
                    if (i == 0)
