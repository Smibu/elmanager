...
namespace Elmanager.EditorTools
{
    internal class TransformTool : ToolBase, IEditorTool
    {
        ...
        private List<Level.Object> _originalTransformObjects;
        private List<Polygon> _originalTransformPolygons;
        private List<Level.Picture> _originalTransformTextures;
        private int _transformPolygonIndex;
        private Polygon _transformRectangle;

        internal TransformTool(LevelEditor editor)
            : base(editor)
        {
        }
        ...
        public void Activate()
        {
            ...
            _originalTransformObjects = new List<Level.Object>();
            _originalTransformTextures = new List<Level.Picture>();
            double xMin = 0;
            double xMax = 0;
            double yMin = 0;
            double yMax = 0;
            ...
            foreach (Polygon x in Lev.Polygons)
            {
                ...
                {
                    if (z.Mark != Geometry.VectorMark.Selected) continue;
                    if (!foundMarked)
                    {
        ...
        public void MouseMove(Vector p)
        {
            ...
            {
                Level.Picture x = _originalTransformTextures[_originalTransformTextures.Count - 1 - i];
        ...
        private void EndTransforming()
        {
            ...
            for (int i = 0; i < _originalTransformObjects.Count; i++)
            {
                Level.Object x = Lev.Objects[Lev.Objects.Count - 1 - i];
