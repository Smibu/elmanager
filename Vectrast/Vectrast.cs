...
namespace vectrast
{
    internal class VectRast
    {
        ...
        private static int Main(string[] args)
        {
            ...
            var printProgressOn = true;
            var printWarningsOn = false;
            var load_type = IOType.None;
            var save_type = IOType.None;
            ...
            try
            {
                while (arg_num < args.Length)
                {
                    ...
                    switch (arg_now)
                    {
                        ...
                        case "-translate":
                            // offset by tx, ty
                            var tx = double.Parse(args[arg_num++]);
                            var ty = double.Parse(args[arg_num++]);
                            transformMatrix = transformMatrix*Matrix2D.translationM(tx, ty);
                            break;
                        case "-rotate":
                            // rotate by angle (in degrees)
                            var ang = double.Parse(args[arg_num++]);
                            transformMatrix = transformMatrix*Matrix2D.rotationM(ang);
                            break;
                        case "-scale":
                            // scale by factor sx, sy
                            var sx = double.Parse(args[arg_num++])*ONEPIXEL;
                            var sy = double.Parse(args[arg_num++])*ONEPIXEL;
                            transformMatrix = transformMatrix*Matrix2D.scaleM(sx, sy);
                            break;
                        case "-flowers":
                            // number of flowers
                            numFlowers = int.Parse(args[arg_num++]);
                            break;
                        case "-progress":
                            // show progress continually
                            printProgressOn = bool.Parse(args[arg_num++]);
                            break;
                        case "-warnings":
                            // print warnings
                            printWarningsOn = bool.Parse(args[arg_num++]);
                            break;
                        default:
            ...
            var vr = new VectRast(printProgressOn, printWarningsOn);
            if (load_type == IOType.Bitmap || load_type == IOType.LevelBitmap)
            {
                ...
                try
                {
                    ...
                    vr.loadAsBmp(loadBmpFileName, out bmp, out pixelOn,
                        ...
                        load_type == IOType.LevelBitmap ? (byte) 0 : (byte) 1, numFlowers);
                }
                catch (Exception e)
                {
            ...
            switch (save_type)
            {
                ...
                case IOType.Level:
                    try
                    {
                        ...
                        vr.saveAsLev(saveLevFileName, "autogenerated on " + DateTime.Now.ToString("g"), "DEFAULT",
                            "ground", "sky");
                    }
                    catch (Exception e)
                    {
